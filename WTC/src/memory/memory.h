...
public:
	...
	*/
	...
	/**
	 * \brief Fully resets current DMA/process context and closes active VMM handle.
	 */
	void Disconnect();
	...
	*/
	...
	bool AddScatterWriteRequest(VMMDLL_SCATTER_HANDLE handle, uint64_t addr, T value) const
	{
		if (!handle || !addr)
			return false;
		return VMMDLL_Scatter_PrepareWrite(handle, addr, reinterpret_cast<PBYTE>(&value), sizeof(value)) != 0;
	}
...
inline const std::unique_ptr< c_memory > TargetProcess{ new c_memory( ) };
