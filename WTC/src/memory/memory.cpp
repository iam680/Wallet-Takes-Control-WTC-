...
namespace
{
    constexpr uintptr_t k_min_user_address = 0x2000000;
    constexpr uintptr_t k_max_user_address = 0x7FFFFFFFFFFF;

    inline bool is_valid_user_address( const uintptr_t address )
    {
        return address > k_min_user_address && address < k_max_user_address;
    }

    inline bool should_log_transient_failure()
    {
...
bool c_memory::Init(std::string process_name, bool memMap, bool debug)
{
	...
	{
		const DWORD current_pid = GetPidFromName( process_name );
		if ( current_pid != 0 && current_pid == this->current_process.PID )
		{
			LOG("Process already initialized!\n");
			return true;
		}

		LOG("[!] Process context is stale. Reinitializing...\n");
		PROCESS_INITIALIZED = FALSE;
		this->current_process = { };
		this->Modules.clear( );
	}
	...
	LOG("Base Address: 0x%zx\n", this->current_process.base_address);
	LOG("Base Size: 0x%zx\n", this->current_process.base_size);
	// static bool dma_debug = []() {
	// 	LOG("[DMA] VMM Handle    : 0x%p\n", TargetProcess->vHandle);
	// 	LOG("[DMA] Process PID   : %d\n", TargetProcess->current_process.PID);
	// 	LOG("[DMA] CR3 fixed     : %s\n", TargetProcess->current_process.base_address ? "YES" : "NO");
	// 	return true;
	// 	}();
...
size_t c_memory::GetBaseAddress(std::string module_name)
{
	...
	if (!Modules.contains(str))
	{
		...
		LOG("[+] Found Base Address for %s at 0x%zx\n", module_name.c_str(), module_info->vaBase);
...
{
	if ( !is_valid_user_address( address ) || !buffer || size == 0 )
		return false;
	if ( !VMMDLL_MemWrite( this->vHandle, this->current_process.PID, address, ( PBYTE )buffer, size ) )
	{
		if ( should_log_transient_failure() )
			LOG("[!] Failed to write Memory at 0x%zx\n", address);
		return false;
	}
	return true;
}
...
{
	if ( !is_valid_user_address( address ) || !buffer || size == 0 || pid <= 0 )
		return false;
...
bool c_memory::Read(uintptr_t address, void* buffer, size_t size) const
{
	...
	{
		if ( should_log_transient_failure() )
			LOG("[!] Failed to read Memory at 0x%zx\n", address);
		return false;
	}
	return true;
}
...
{
	if ( !handle || !is_valid_user_address( static_cast<uintptr_t>( address ) ) || !buffer || size == 0 )
		return false;

	DWORD memoryPrepared = 0;
	return VMMDLL_Scatter_PrepareEx( handle, address, size, ( PBYTE )buffer, &memoryPrepared ) != 0;
}
...
bool c_memory::AddScatterWriteRequest(VMMDLL_SCATTER_HANDLE handle, uint64_t address, void* buffer, size_t size)
{
	...
	return VMMDLL_Scatter_PrepareWrite( handle, address, ( PBYTE )buffer, size ) != 0;
}
...
bool c_memory::ExecuteScatterWrite(VMMDLL_SCATTER_HANDLE handle, bool bClear)
{
	...
	if ( bClear )
		return VMMDLL_Scatter_Clear( handle, this->current_process.PID, VMMDLL_FLAG_NOCACHE ) != 0;
